...
const AddLocalPurchase: React.FC = () => {
    ...
    const [totalVds, setTotalVDS] = useState('');
        ...
        if (dataTable) {
            dataTable.querySelectorAll('tr').forEach((row) => {
                ...
                row.querySelectorAll('td input').forEach((input) => {
                    ...
                    rowData[inputElement.name || 'invoiceNo'] = inputElement.value;
                    rowData[inputElement.name || 'challanDate'] = inputElement.value;
                    rowData[inputElement.name || 'amount'] = inputElement.value;
                    rowData[inputElement.name || 'vatAmount'] = inputElement.value;
                    rowData[inputElement.name || 'deductedVat'] = inputElement.value;
                    rowData[inputElement.name || 'receiveAmount'] = inputElement.value;
                    rowData[inputElement.name || 'accountCode'] = inputElement.value;
                    rowData[inputElement.name || 'bankChallanSerial'] = inputElement.value;
                            ...
                            <form className="space-y-5" >
                                <div className="grid grid-cols-1 md:grid-cols-6 gap-5">
                                    ...
                                    <div>
                                        ...
                                        <input id="browserLname" type="date" placeholder="" className="form-input" onChange={(e) => setReceivedDate(e.target.value)} />
                                    </div>
                                </div>
                                <div className="border overflow-hidden overflow-x-auto">
                                    <table id="dataTable" className="whitespace-nowrap table-hover border dataTable">
                                        <thead>
                                            <tr className="whitespace-nowrap border overflow-x-auto">
                                                <th className="w-9">Invoice No</th>
                                                <th className="w-14" >Challan Date</th>
                                                <th className="w-9 border-black" >Amount</th>
                                                <th className="w-9" >Vat Amount</th>
                                                <th className="w-10 border-x-1 border-black" >Deducted Vat</th>
                                                <th className="w-6" >Receive Amount</th>
                                                <th className="w-9 border-x-1 border-black" >Account Code</th>
                                                <th className="w-14" >Bank Challan Serial</th>
                                    ...
                                    <table className='mt-4'>
                                        <tr className="h-10 border border-black form-input">
                                            ...
                                            <td align="left"><strong><input type='text' id='vatTotal' onChange={(e) => setTotalAmount(e.target.value)} disabled /></strong></td>
                                        </tr>
                                        <tr className="h-10 border border-black form-input">
                                        ...
                                        <tr className="h-10 border border-black form-input">
                                            ...
                                            <td align="left"><strong id=""><input type='text' id='sdTotal' onChange={(e) => setTotalVDS(e.target.value)} disabled /></strong></td>
                                        </tr>
                                        <tr className="h-10 border border-black form-input">
                                            <td className="border-r-2 w-3/5" align="right"><strong>Total Receive(BDT)</strong></td>
                                            <td align="left"><strong><input type="text" id="grandTotal" onChange={(e) => setReceiveTotal(e.target.value)} disabled /></strong>
                                                <p></p>
                                            </td>
                                        </tr>
                                    </table>
